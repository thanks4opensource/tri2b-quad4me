define tri2b
    ...
    printf "data              : %d -> %d\n", \
           (bool)(GPIOB->ODR & tri2b_config::DATA_GPIO_BIT), \
           (bool)(GPIOB->IDR & tri2b_config::DATA_GPIO_BIT)
    printf "alrt              : %d -> %d\n", \
           (bool)(GPIOB->ODR & tri2b_config::ALRT_GPIO_BIT), \
           (bool)(GPIOB->IDR & tri2b_config::ALRT_GPIO_BIT)
    printf "ltch              : %d -> %d\n", \
           (bool)(GPIOB->ODR & tri2b_config::LTCH_GPIO_BIT), \
           (bool)(GPIOB->IDR & tri2b_config::LTCH_GPIO_BIT)
    printf "EXTI->IMR (alrt)  : 0x%02x %c\n"	          , \
	         EXTI->IMR && tri2b_config::ALRT_EXTI_BIT,  \
	         EXTI->IMR && tri2b_config::ALRT_EXTI_BIT   \
	      == tri2b_config::ALRT_GPIO_BIT		    \
	    ? 'A'					    \
            : '.'
    printf "EXTI->IMR (ltch)  : 0x%02x %c\n"	   	  , \
	         EXTI->IMR && tri2b_config::LTCH_EXTI_BIT,  \
	         EXTI->IMR && tri2b_config::LTCH_EXTI_BIT   \
	      == tri2b_config::LTCH_GPIO_BIT		    \
	    ? 'L'				            \
            : '.'

    printf "EXTI (RTSR)       : 0x%02x/%c%c\n"			       , \
	   EXTI->RTSR						       , \
           EXTI->RTSR & tri2b_config::ALRT_EXTI_BIT ? 'A' : '-'        , \
           EXTI->RTSR & tri2b_config::LTCH_EXTI_BIT ? 'L' : '-'
    printf "EXTI (FTSR)       : 0x%02x/%c%c\n"			       , \
	   EXTI->FTSR						       , \
           EXTI->FTSR & tri2b_config::ALRT_EXTI_BIT ? 'A' : '-'	       , \
           EXTI->FTSR & tri2b_config::LTCH_EXTI_BIT ? 'L' : '-'
    printf "EXTI (PR)         : 0x%02x/%c%c\n"			       , \
	   EXTI->PR						       , \
           EXTI->PR & tri2b_config::ALRT_EXTI_BIT ? 'A' : '-'	       , \
           EXTI->PR & tri2b_config::LTCH_EXTI_BIT ? 'L' : '-'
    printf "EXTI (IMR)        : 0x%02x/%c%c\n"    		       , \
	   EXTI->IMR						       , \
           EXTI->IMR & tri2b_config::ALRT_EXTI_BIT ? 'A' : '-'         , \
           EXTI->IMR & tri2b_config::LTCH_EXTI_BIT ? 'L' : '-'
end     
...
\
arm::SYS_TICK_PTR->CTRL & SysTick_CTRL_COUNTFLAG_Msk ? 'r' : '.', \
arm::SYS_TICK_PTR->VAL, \
arm::SYS_TICK_PTR->LOAD, \
meta, \
data, \
num_sends, \
num_recvs, \
...
define results
    ...
    output randomtest::MCU_STRING
    echo \n
    printf "CLOCK_DIV    : 0x%x\n", mcu_config::HPRE_DIV
    printf "CLOCK_KHZ    : %d\n",   mcu_config::MAIN_CLOCK_KHZ
    printf "CLOCK_MHZ    : %d\n",   mcu_config::MAIN_CLOCK_MHZ
    printf "TIM_PRESCALE : %d\n",   randomtest::TIM_PRESCALE
    printf "TIM_KHZ      : %d\n",   randomtest::TIM_KHZ
...
"L %d%d %s-%s-%s %c%c%c->%c%c%c %c%c%c%c %02d/%02d %08x-%08x-%08x %x:%02d:%-2d  %2d/%c/0x%08x\n", \
_NODE_ID, \
_rank, \
...
_ROLE_STRINGS [_role ], \
\
(bool)(GPIOB->ODR & tri2b_config::DATA_GPIO_BIT) ? 'd' : '.', \
(bool)(GPIOB->ODR & tri2b_config::ALRT_GPIO_BIT) ? 'a' : '.', \
(bool)(GPIOB->ODR & tri2b_config::LTCH_GPIO_BIT) ? 'l' : '.', \
(bool)(GPIOB->IDR & tri2b_config::DATA_GPIO_BIT) ? 'd' : '.', \
(bool)(GPIOB->IDR & tri2b_config::ALRT_GPIO_BIT) ? 'a' : '.', \
(bool)(GPIOB->IDR & tri2b_config::LTCH_GPIO_BIT) ? 'l' : '.', \
\
EXTI->IMR  & tri2b_config::ALRT_EXTI_BIT ? 'I' : '.' , \
EXTI->RTSR & tri2b_config::ALRT_EXTI_BIT ? 'R' : '.' , \
EXTI->FTSR & tri2b_config::ALRT_EXTI_BIT ? 'F' : '.' , \
EXTI->PR   & tri2b_config::ALRT_EXTI_BIT ? 'P' : '.', \
\
